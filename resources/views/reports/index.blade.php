...
@section('content')
    <div class="sm:flex sm:items-center sm:justify-between mb-8">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">Realtime breakdown of the Helpdesk system workload, assignments, and
                structural data.</p>
        </div>
    </div>
                    ...
                    <ul class="space-y-4">
                        @forelse($ticketsByCategory as $cat)
                            <li class="flex items-center justify-between">
                                ...
                                <span
                                    class="inline-flex items-center rounded-full bg-gray-100 px-3 py-1 text-xs font-bold text-gray-600 border border-gray-200">
                                    {{ $cat->tickets_count }}
                                </span>
                            </li>
                        @empty
                    ...
                    <div class="p-6 grid grid-cols-2 sm:grid-cols-3 gap-4">
                        @foreach($ticketsByStatus as $statusData)
                            <div class="rounded-lg border border-dashed border-gray-200 p-4 text-center">
                                ...
                                <div class="text-xs uppercase mt-1 font-medium text-gray-500">
                                    {{ str_replace('_', ' ', $statusData->status) }}</div>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="rounded-xl border border-gray-200 bg-white overflow-hidden shadow-sm">
                    ...
                    <div class="p-6 grid grid-cols-3 gap-4">
                        @foreach($ticketsByPriority as $priorityData)
                            <div
                                class="rounded-lg p-3 text-center {{ $priorityData->priority == 'high' ? 'bg-red-50 text-red-700' : ($priorityData->priority == 'medium' ? 'bg-blue-50 text-blue-700' : 'bg-gray-100 text-gray-600') }}">
